...
export class SalidasService {
  ...
  async findByAlmacen(
      ...
      baseQB.andWhere(
        new Brackets((qb) => {
          qb.where('almacen.name ILIKE :term', { term })
            .orWhere('s.prestadaPara ILIKE :term', { term })
            .orWhere('producto.name ILIKE :term', { term })
    ...
    const salidas = await this.salidaRepo
      .createQueryBuilder('s')
      .select(['s.id', 's.fecha', 's.prestadaPara'])
      .leftJoin('s.almacenOrigen', 'almacen')
