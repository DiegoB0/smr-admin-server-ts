...
export class ProductosService {
  ...
  */
  async createProduct(dto: CreateProductoDto, user: User): Promise<Producto> {
    const { customId, name, description, unidad } = dto

    const producto = await this.productRepo.findOne({ where: { name } })
  ...
  private async initialInventory(productId: number): Promise<void> {
